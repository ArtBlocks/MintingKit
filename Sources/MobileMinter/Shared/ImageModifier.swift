...
//  MobileMinter
//
//  Created by Shantanu Bala on 11/17/22.
//
import SwiftUI
import UIKit
...
struct ImageModifier: ViewModifier {
  ...
  func body(content: Content) -> some View {
    ScrollView([.horizontal, .vertical]) {
      content
        .frame(
          width: contentSize.width * currentScale, height: contentSize.height * currentScale,
          alignment: .center
        )
...
struct PinchZoom: UIViewRepresentable {
  ...
  func makeUIView(context: Context) -> PinchZoomView {
    let pinchZoomView = PinchZoomView(
      minScale: minScale, maxScale: maxScale, currentScale: scale, scaleChange: { scale = $0 })
...
struct PinchToZoom: ViewModifier {
  ...
  func body(content: Content) -> some View {
    content
      ...
      .overlay(
        PinchZoom(minScale: minScale, maxScale: maxScale, scale: $scale, isPinching: $isPinching))
  }
}
