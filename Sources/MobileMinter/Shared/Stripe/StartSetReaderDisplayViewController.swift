...
class StartSetReaderDisplayViewController: TableViewController {
  ...
  lazy var taxSection = Section(
    header: Section.Extremity.title("Tax: \(self.taxView.amountString)"), rows: [],
    footer: Section.Extremity.autoLayoutView(taxView))
  lazy var totalSection = Section(
    header: Section.Extremity.title("Total: \(self.totalView.amountString)"), rows: [],
  ...
  internal func startSetReaderDisplay() {
    let cart = Cart(
      currency: currencyView.currency, tax: Int(taxView.amount), total: Int(totalView.amount))
  ...
  internal func addLineItem() {
    let alertController = UIAlertController(
      title: "Add New Line Item", message: "", preferredStyle: UIAlertController.Style.alert)
    ...
    let createAction = UIAlertAction(
      title: "Create", style: UIAlertAction.Style.default,
  ...
  private func updateContent() {
    let currencySection = Section(
      header: "Currency", rows: [],
      footer: Section.Extremity.autoLayoutView(currencyView))

    var lineItemsRows: [Row] = []
    for lineItem in self.lineItems {
      let row = Row(
        text: lineItem.displayName, detailText: "\(lineItem.amount) ×\(lineItem.quantity)",
        selection: .none, image: nil, accessory: .none, cellClass: SubtitleCell.self, context: nil,
        editActions: [
          .init(
            title: "Delete", style: .normal, backgroundColor: .systemRed, backgroundEffect: nil,
