  ...
  var description: String? {
    ...
    case .paymentIntent(let intent):
      return !intent.originalJSON.isEmpty
        ? prettyPrint(json: intent.originalJSON) : intent.debugDescription
...
class LogEventViewController: TableViewController {
  ...
  override func viewDidLoad() {
    ...
    let methodSection = Section(
      header: "METHOD", rows: [], footer: Section.Extremity.autoLayoutView(methodView))
    sections.append(methodSection)
    let resultView = MonospaceTextView(text: event.resultDetail)
    let resultSection = Section(
      header: "RESULT", rows: [], footer: Section.Extremity.autoLayoutView(resultView))
    sections.append(resultSection)
    if let status = event.paymentIntentStatus {
      let view = MonospaceTextView(text: status)
      let section = Section(
        header: "STATUS", rows: [], footer: Section.Extremity.autoLayoutView(view))
    ...
    if let description = event.object.description {
      ...
      let section = Section(
        header: header, rows: [], footer: Section.Extremity.autoLayoutView(view))
      sections.append(section)
    }
