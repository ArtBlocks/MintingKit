...
extension UIViewController {
  ...
  func presentValuePicker<T>(
    title: String? = nil, options: [T], from sourceView: UIView? = nil,
    handler: @escaping (T?) -> Void
  ) where T: CustomStringConvertible {
    guard canPresentViewController() else { return }

    let alertController = UIAlertController(
      title: title, message: nil, preferredStyle: sourceView == nil ? .alert : .actionSheet)
    for opt in options {
      let action = UIAlertAction(
        title: opt.description, style: .default,
        handler: { _ in
          handler(opt)
        })
  ...
  /**
     ...
     */
  func presentAlert(
    title: String, message: String, okButtonTitle: String = "OK", handler: @escaping (Bool) -> Void
  ) {
    guard canPresentAlert() else {
      print("‼️ \(title)")
      print(message)
