...
class StartPaymentViewController: TableViewController, CancelingViewController {
  ...
  private let applicationFeeAmountTextField = TextFieldView(
    placeholderText: "Application Fee Amount")
  ...
  internal func startPayment() {
    let captureMethod =
      self.automaticCaptureEnabled ? CaptureMethod.automatic : CaptureMethod.manual
    ...
    let collectConfig = CollectConfiguration(
      skipTipping: self.skipTipping, updatePaymentIntent: declineCardBrand != nil)
  ...
  private func makePaymentMethodOptionsParameters() -> PaymentMethodOptionsParameters {
    ...
    if self.manualPreferredEnabled {
      cardPresentParams.captureMethod = NSNumber(
        value: CardPresentCaptureMethod.manualPreferred.rawValue)
    }
    ...
    if requestedPriority == "International" {
      cardPresentParams.requestedPriority = NSNumber(
        value: CardPresentRouting.international.rawValue)
    }
  ...
  private func makePaymentMethodSection() -> Section {
    ...
    let paymentMethodSection = Section(
      ...
      rows: [
        Row(
          text: "Enable Interac Present",
          accessory: .switchToggle(value: self.interacPresentEnabled) { [unowned self] _ in
        ...
        Row(
          ...
          selection: { [unowned self] in
            ...
            self.presentValuePicker(
              options: ["None"] + brands.map { Terminal.stringFromCardBrand($0) }
            ) { picked in
        ...
        Row(
          ...
          accessory: .switchToggle(value: self.requestIncrementalAuthorizationSupport) {
            [unowned self] _ in
            self.requestIncrementalAuthorizationSupport.toggle()
            self.updateContent()
          }),
      ],
      footer: shouldShowTestCardPickerView
        ? Section.Extremity.autoLayoutView(TestCardPickerView()) : nil)
