...
class ReaderViewController: TableViewController, CancelingViewController {
  ...
  private var reconnectionAlertController = UIAlertController(
    title: "Reconnecting...", message: "Reader has disconnected", preferredStyle: .alert)
  ...
  override func viewDidLoad() {
    ...
    headerView.addGestureRecognizer(
      UITapGestureRecognizer(target: self, action: #selector(didTapReaderHeaderView)))
  ...
  internal func showDiscoveryMethods() {
    let vc = DiscoveryMethodViewController(
      method: ReaderViewController.readerConfiguration.discoveryMethod)
  ...
  internal func updateContent() {
    ...
    if let connectedReader = self.connectedReader {
      var workflowRows = [
        Row(
          text: "Collect card payment", detailText: "Collect a payment by reading a card",
          selection: { [unowned self] in
            self.showStartPayment()
          }, accessory: .disclosureIndicator, cellClass: SubtitleCell.self),
        Row(
          text: "Store card via readReusableCard",
          detailText: "Create a payment method for future use.",
          selection: { [unowned self] in
      ...
      case .verifoneP400, .wisePosE, .wisePosEDevKit, .etna, .stripeS700, .stripeS700DevKit:
        workflowRows.append(
          Row(
            text: "Set reader display", detailText: "Display an itemized cart on the reader",
            selection: { [unowned self] in
        ...
        && connectedReader.deviceType != .appleBuiltIn
      {
        workflowRows.append(
          Row(
            text: "In-Person Refund", detailText: "Refund a charge made by an Interac debit card.",
            selection: { [unowned self] in
