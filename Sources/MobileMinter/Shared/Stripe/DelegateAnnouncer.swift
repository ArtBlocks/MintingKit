...
class BluetoothReaderDelegateAnnouncer: DelegateAnnouncer<BluetoothReaderDelegate>,
  BluetoothReaderDelegate
{
  ...
  func reader(
    _ reader: Reader, didReportBatteryLevel batteryLevel: Float, status: BatteryStatus,
    isCharging: Bool
  ) {
    announce { delegate in
      delegate.reader?(
        reader, didReportBatteryLevel: batteryLevel, status: status, isCharging: isCharging)
    }
  }
...
class LocalMobileReaderDelegateAnnouncer: DelegateAnnouncer<LocalMobileReaderDelegate>,
