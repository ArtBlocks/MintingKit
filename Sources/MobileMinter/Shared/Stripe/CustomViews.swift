...
class CurrencyInputView: TextFieldView, UIPickerViewDelegate, UIPickerViewDataSource {
  ...
  convenience init() {
    ...
    let currency =
      (UIApplication.shared.delegate as? AppDelegate)?.defaultCurrency.uppercased() ?? "USD"
    ...
    -> String?
  {
    ...
    if let localizedString = NSLocale.current.localizedString(
      forCurrencyCode: currencyCode.uppercased())
    {
...
class CountryInputView: TextFieldView, UIPickerViewDelegate, UIPickerViewDataSource {
  ...
  convenience init() {
    ...
    textField.text = generateLabel(
      forCountryCode: StripeCountries.supportedByTerminal.first ?? "US")
    textField.keyboardType = .alphabet
    pickerView.dataSource = self
    pickerView.delegate = self
    pickerView.backgroundColor = UIColor.white
...
class ReaderUpdatePicker: TextFieldView, UIPickerViewDelegate, UIPickerViewDataSource {
  ...
  convenience init() {
    ...
    pickerView.selectRow(
      updateTypesInDisplayOrder.firstIndex(of: initialSelectedReaderUpdate) ?? 0, inComponent: 0,
      animated: false)
  }
...
class TextFieldView: UIView {
  ...
  init(
    text: String? = nil, header: String? = nil, footer: String? = nil,
...
class LinkTextView: UIView {
  ...
  static func buildAttributedString(prefix: String, linkName: String, urlString: String)
    -> NSAttributedString
  {
  ...
  init(prefix: String, linkName: String, urlString: String) {
    ...
    textView.attributedText = LinkTextView.buildAttributedString(
      prefix: prefix, linkName: linkName, urlString: urlString)
    textView.sizeToFit()
