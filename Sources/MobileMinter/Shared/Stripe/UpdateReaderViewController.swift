...
class UpdateReaderViewController: TableViewController, CancelableViewController {
  ...
  convenience init(update: ReaderSoftwareUpdate, updateInstalledCompletion: @escaping () -> Void) {
    self.init(
      update: update, type: .available, updateInstalledCompletion: updateInstalledCompletion)
  ...
  convenience init(
    updateBeingInstalled: ReaderSoftwareUpdate, cancelable: Cancelable?,
    updateInstalledCompletion: @escaping () -> Void
  ) {
    self.init(
      update: updateBeingInstalled, type: .required,
  ...
  private init(
    update: ReaderSoftwareUpdate, type: UpdateType, updateInstalledCompletion: @escaping () -> Void
  ) {
    self.update = update
  ...
  override func viewDidLoad() {
    ...
    let cancelButton = UIBarButtonItem(
      title: "Cancel", style: .plain, target: self, action: #selector(cancelAction))
  ...
  private func updateContent() {
    ...
    if let progress = updateProgress {
      ...
      updateRow = Row(
        text: "⏱ \(updateType == .required ? "Required update" : "Update") in progress")
  ...
  func updateVersionText() -> String {
    ...
    return
      "Components to update: \(components.joined(separator: ","))\n\nEstimated update time: \(updateEstimate)"
  }
}
  ...
  func reader(
    ...
    if error == nil {
      self.presentAlert(
        ...
        message:
          "The reader has been updated to \(update?.deviceSoftwareVersion ?? "no update available")."
      )
