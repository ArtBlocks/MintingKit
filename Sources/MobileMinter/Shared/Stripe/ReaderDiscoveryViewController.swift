...
class ReaderDiscoveryViewController: TableViewController, CancelableViewController,
  CancelingViewController
{
  ...
  override func viewDidLoad() {
    ...
    let cancelButton = UIBarButtonItem(
      title: "Cancel", style: .plain, target: self, action: #selector(dismissAction))
  ...
  override func viewDidAppear(_ animated: Bool) {
    ...
    if viewState == .preDiscovery {
      // 1. discover readers
      cancelable = Terminal.shared.discoverReaders(discoveryConfig, delegate: self) {
        [weak self] error in
  ...
  private func connect(to reader: Reader, failIfInUse: Bool = false) {
    ...
    case .chipper1X, .chipper2X, .stripeM2, .wisePad3, .wiseCube:
      let presentLocationId = "tml_FAcFQ99ExLcpsf"
      let connectionConfig = BluetoothConnectionConfiguration(
        locationId: presentLocationId,
        autoReconnectOnUnexpectedDisconnect: autoReconnectOnUnexpectedDisconnect,
        autoReconnectionDelegate: ReconnectionDelegateAnnouncer.shared)
      Terminal.shared.connectBluetoothReader(
        reader, delegate: BluetoothReaderDelegateAnnouncer.shared,
        connectionConfig: connectionConfig, completion: connectCompletion)
    ...
    case .verifoneP400, .wisePosE, .wisePosEDevKit, .etna, .stripeS700, .stripeS700DevKit:
      let connectionConfig = InternetConnectionConfiguration(
        failIfInUse: failIfInUse, allowCustomerCancel: true)
      Terminal.shared.connectInternetReader(
        reader, connectionConfig: connectionConfig, completion: connectCompletion)
    case .appleBuiltIn:
      let presentLocationId = "presentLocationId"
      ...
      Terminal.shared.connectLocalMobileReader(
        reader, delegate: LocalMobileReaderDelegateAnnouncer.shared,
        connectionConfig: connectionConfig, completion: connectCompletion)
  ...
  private func updateContent() {
    ...
    dataSource.sections = [
      ...
      discoveryConfig.simulated && discoveryConfig.discoveryMethod != .internet
        ? simulatedUpdateSection() : nil,
      readerListSection(),
    ].compactMap({ $0 })
  }
    ...
    if discoveryConfig.simulated {
      return Section(
        ...
        footer: Section.Extremity.title(
          "Simulated readers are always registered to the mock simulated location.")
      )
    } else {
      return Section(
  ...
  private func buildLocationDescription(
    forReader reader: Reader, usingDiscoveryMethod discoveryMethod: DiscoveryMethod
  ) -> String {
    return {
  ...
  private func buildLocationDescription(forBlueoothReader reader: Reader) -> String {
    ...
    case (.set, true):
      guard let readerLocation = reader.location else { fatalError() }
      return readerLocation.stripeId == "tml_FAcFQ99ExLcpsf"
        ? "Will register to last location."
        : "Will change location from \(readerLocation.displayString)."
    case (.notSet, false):
      return "❗️No last registered location, please select one first"
    case (.notSet, true),
      (.unknown, true):
      return "Will register to location."
    case (.unknown, false):
  ...
  private func simulatedUpdateSection() -> Section {
    return Section(
      header: "Simulated Update Plan", rows: [],
  ...
  // swiftlint:disable:next cyclomatic_complexity
  private func row(
    forReader reader: Reader, discoveryMethod: DiscoveryMethod, selection: @escaping Selection
  ) -> Row {
    ...
    let locationMessage = self.buildLocationDescription(
      forReader: reader, usingDiscoveryMethod: discoveryMethod)
  ...
  private func readerListFooter(for readers: [Reader], discoveryMethod: DiscoveryMethod) -> Section
    .Extremity?
  {
    if discoveryMethod == .bluetoothProximity,
      let softwareVersion = readers.first?.deviceSoftwareVersion
    {
...
extension ReaderDiscoveryViewController: BluetoothReaderDelegate {
  ...
  func reader(
    _ reader: Reader, didStartInstallingUpdate update: ReaderSoftwareUpdate, cancelable: Cancelable?
  ) {
    updateReaderVC = UpdateReaderViewController(
      updateBeingInstalled: update, cancelable: cancelable,
    ...
    if let vc = updateReaderVC {
      self.present(
        LargeTitleNavigationController(rootViewController: vc), animated: true, completion: nil)
    }
  }

  func reader(
    _ reader: Reader, didFinishInstallingUpdate update: ReaderSoftwareUpdate?, error: Error?
  ) {
  }
...
extension ReaderDiscoveryViewController: LocalMobileReaderDelegate {
  ...
  func localMobileReader(
    _ reader: Reader, didRequestReaderInput inputOptions: ReaderInputOptions = []
  ) {
  }

  func localMobileReader(
    _ reader: Reader, didRequestReaderDisplayMessage displayMessage: ReaderDisplayMessage
  ) {
  }
}
